...
suite('Functional Tests', function() {
  ...
  suite('Integration tests with chai-http', function() {
    ...
    suite('GET /hello?name=[name] => "hello [name]"', function(){
      ...
      test('Test GET /hello with your name',  function(done){ // Don't forget the callback...
         chai.request(server)             // 'server' is the Express App
          .get('/hello?name=xy_z')         /** Put your name in the query **/ // http_method(url)
          .end(function(err, res){        // res is the response object
    ...
    suite('PUT /travellers', function(){
      ...
      // Now it's your turn. Make the test pass. We expect the response to be
      // {name: 'Cristoforo', surname: 'Colombo', dates: '1451 - 1506'}
      // check the status, the type, name and surname. 
      // ** Follow the order of the assertions **, we rely on it in our tests.
  ...
  // an environment such as Hyperdev. There are many other (more powerful) options.

  var Browser = require('zombie');

  // On hyperdev we'll use this setting
  Browser.site = 'https://time-shape.gomix.me'; /** Copy your project's url here **/
  // If you are testing on a local environment replace it with the next line
  ...
  suite('e2e Testing with Zombie.js', function() {
    ...
    suite('"Famous Italian Explorers" form', function() {
      ...
      test('submit "surname" : "Colombo" - write your e2e test...', function(done) {
        ...
        // fill the form...
        // then submit pressing 'submit' button.
        //
        // in the callback...
        // assert that status is OK 200
